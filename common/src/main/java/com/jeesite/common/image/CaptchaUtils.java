...
public class CaptchaUtils {
	...
	private static ConfigurableCaptchaService ccs6bit;
	...
	private static ConfigurableCaptchaService initialize(ConfigurableCaptchaService ccs, Integer captchaBit){
		if (ccs == null){
        	synchronized (CaptchaUtils.class) {
        		if (ccs == null){
	        		...
	        		wf.setMinLength(captchaBit);
	        		wf.setMaxLength(captchaBit);
	...
	public static String generateCaptcha(OutputStream outputStream,Integer bit) throws IOException{
		if (bit == 6){
			return generateCaptcha6bit(outputStream);
		}
		return generateCaptcha(outputStream);
	}

	private static String generateCaptcha6bit(OutputStream outputStream)throws IOException {
		// 初始化设置
		if (CaptchaUtils.ccs6bit == null){
			synchronized (CaptchaUtils.class){
				if (CaptchaUtils.ccs6bit == null){
					CaptchaUtils.ccs6bit = initialize(CaptchaUtils.ccs6bit,6);
				}
			}
		}
		...
		switch (random.nextInt(3)) {
			case 0:
				ccs6bit.setFilterFactory(wrff); // 摆波纹
				break;
			case 1:
				ccs6bit.setFilterFactory(doff); // 双波纹
				break;
			case 2:
				ccs6bit.setFilterFactory(crff); // 曲线波纹
				break;
			case 3:
				ccs6bit.setFilterFactory(drff); // 漫纹波
				break;
			case 4:
				ccs6bit.setFilterFactory(mrff); // 大理石
				break;
		}

		// 生成验证码
		String s = EncoderHelper.getChallangeAndWriteImage(ccs6bit, "png", outputStream);
	...
	public static String generateCaptcha(OutputStream outputStream) throws IOException{
		...
		// 初始化设置
		if (CaptchaUtils.ccs == null){
			synchronized (CaptchaUtils.class){
				if (CaptchaUtils.ccs == null){
					CaptchaUtils.ccs = initialize(CaptchaUtils.ccs,4);
				}
			}
		}
	...
	public static void main(String[] args) throws IOException {
		...
		FileOutputStream fos = new FileOutputStream("C:\\Users\\USR\\Desktop\\captcha.png");
		FileOutputStream fos2 = new FileOutputStream("C:\\Users\\USR\\Desktop\\captcha6bit.png");
		String s = generateCaptcha(fos);
		String s2 = generateCaptcha(fos2,6);
