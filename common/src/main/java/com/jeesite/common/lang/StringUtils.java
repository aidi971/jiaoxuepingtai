...
//		{//章节过滤
			String html = "        Excel 应 用 基 础       ";
			String s = trim2(html);
			int pre = s.indexOf("第");
			int su = s.indexOf("章");
			if (su==-1 || su==-1) {
				String substring = s.substring(pre, su + 1);
				String replace = s.replace(substring, "");
				 html = trim2(replace);
			}

			System.out.println("s2 = " + html);
//		}
		//节，小节过滤
//		String html = "     1.1.2 运用Excel进行业务处理的过程1     ";
//		String s = trim2(html);
//		int pre = s.indexOf(" ");
//		String substring = s.substring(0,pre);
//		String replace = s.replace(substring, "");
//		String suString = trim2(replace);
//		System.out.println("s2 = " + suString);
